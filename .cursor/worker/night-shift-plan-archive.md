# Archived Night Shift Plan entries

Older entries moved from night-shift-plan.md to keep the main file small. Last 25 entries remain in night-shift-plan.md.

# Night Shift Plan — 2025-02-18

---

# Night Shift Plan — 2025-02-19 (This run: Test phase recent-projects)

This run implemented the **Test phase: recent-projects** plan below: added `src/lib/__tests__/recent-projects.test.ts` and `.cursor/adr/0325-test-phase-recent-projects.md`. Run **`npm run verify`** locally to confirm.

---

## Night Shift Plan — 2025-02-19 (Test phase: recent-projects)

### Chosen Feature

**Test phase: Add unit tests for recent-projects.** The `recent-projects.ts` module provides `getRecentProjectIds()` and `recordProjectVisit(projectId)` for command-palette ordering (last N project IDs in localStorage). It has no tests. Adding unit tests documents SSR-safe behaviour, localStorage parsing (valid/invalid JSON, non-array, non-string elements), dedupe/max-cap and trim behaviour, and guards against regressions. Does not duplicate parse-first-remote-url, copy-app-version, or print tests already done.

### Approach

- **New tests:** `src/lib/__tests__/recent-projects.test.ts` — mock `window` and `localStorage`. Test `getRecentProjectIds`: returns [] when window undefined (SSR); returns [] for missing/null key or invalid JSON; returns filtered string array up to MAX_RECENT; filters out non-strings. Test `recordProjectVisit`: no-op when window undefined or projectId empty/whitespace; prepends and dedupes; trims projectId; caps at MAX_RECENT; persists to localStorage. Follow existing Vitest + mock patterns (e.g. copy-to-clipboard.test.ts).
- **ADR:** `.cursor/adr/0325-test-phase-recent-projects.md` — document adding tests for recent-projects.
- Run `npm run verify` and fix any failures. Update this plan with Outcome.

### Files to Create

- `src/lib/__tests__/recent-projects.test.ts` — unit tests for getRecentProjectIds and recordProjectVisit (mocked localStorage/window).
- `.cursor/adr/0325-test-phase-recent-projects.md` — ADR for this test-phase addition.
...
- [x] Create `src/lib/__tests__/recent-projects.test.ts` with tests for getRecentProjectIds and recordProjectVisit.
- [x] Create ADR `.cursor/adr/0325-test-phase-recent-projects.md`.
- [ ] Run `npm run verify` and fix any failures (run locally by developer).
- [x] Update this plan with Outcome section.

### Outcome

**What was built** — Unit tests for `recent-projects.ts`: `getRecentProjectIds()` and `recordProjectVisit(projectId)`, used by the command palette to order projects by recent visits (localStorage). `src/lib/__tests__/recent-projects.test.ts` uses an in-memory mock storage (no vi.fn()) and covers: getRecentProjectIds — returns [] when window is undefined (SSR), when key is missing or invalid JSON or value is not an array; returns only string elements capped at 10; returns valid string array when storage has valid JSON. recordProjectVisit — no-op when window undefined or projectId empty/whitespace; prepends and dedupes; trims projectId; caps at 10 and persists. No production code changes.

**Files created:** `src/lib/__tests__/recent-projects.test.ts`, `.cursor/adr/0325-test-phase-recent-projects.md`.
...
- **ADR:** `.cursor/adr/0324-test-phase-parse-first-remote-url.md` — document adding tests for parse-first-remote-url.
- Run `npm run verify` and fix any failures. Update this plan with Outcome.

### Files to Create

- `src/lib/__tests__/parse-first-remote-url.test.ts` — unit tests for parseFirstRemoteUrl.
- `.cursor/adr/0324-test-phase-parse-first-remote-url.md` — ADR for this test-phase addition.
...
- [x] Create ADR `.cursor/adr/0324-test-phase-parse-first-remote-url.md`.
- [ ] Run `npm run verify` and fix any failures (run locally by developer).
- [x] Update this plan with Outcome section.
...
**Files created:** `src/lib/__tests__/parse-first-remote-url.test.ts`, `.cursor/adr/0324-test-phase-parse-first-remote-url.md`.
