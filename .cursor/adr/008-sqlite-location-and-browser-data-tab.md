...
- **Browser Data tab**: Add `GET /api/data/files` that returns `{ scripts, jsonFiles }` by reading `script/` and `data/*.json` from the project (Node `fs`, `path` with `process.cwd()`). Add `GET /api/data/file?path=...` to return file content for a path under project root (path sanitized, no `..`). When not in Tauri, the Data tab loads scripts and JSON file list from these APIs and can show file content via the file API. KV store in browser can be derived from the same JSON (all_projects, cursor_projects) returned by `/api/data` or a small extension.
