...
`run_prompts_all_projects.sh` opens each Cursor project from `cursor_projects.json`, opens the Composer (Agent) panel, pastes the **combined content** of selected prompts from `prompts-export.json`, and presses Enter. It then waits 330 seconds and repeats until you stop it (Ctrl+C).
...
2. **Wait**: Sleeps 330 seconds (`SLEEP_BETWEEN_ROUNDS`, override with env var).
...
- `SLEEP_BETWEEN_ROUNDS` – seconds between rounds (default: 330)
...
- `AGENT_PANEL_DOUBLE_I` – 1 = send Cmd+I twice so panel stays open (default: 1)
