...
describe("cursor-paths constants", () => {
  ...
  it("project prompt path is under project root", () => {
    expect(PROJECT_PROMPT_PATH).toBe(".cursor/1. project/project.prompt.md");
    expect(PROJECT_PROMPT_PATH.startsWith(PROJECT_ROOT)).toBe(true);
  });
  ...
  it("planner and ideas doc paths are defined", () => {
    ...
    expect(IDEAS_PROMPT_PATH).toBe(".cursor/0. ideas/ideas.prompt.md");
  });
});
...
describe("SETUP_* constants", () => {
  it("setup design, architecture, testing paths are under 1. project", () => {
    ...
    expect(SETUP_DESIGN_PROMPT_PATH).toBe(".cursor/1. project/design.prompt.md");
    expect(SETUP_ARCHITECTURE_DOC_PATH).toBe(".cursor/1. project/architecture.md");
    expect(SETUP_ARCHITECTURE_PROMPT_PATH).toBe(".cursor/1. project/architecture.prompt.md");
    expect(SETUP_TESTING_DOC_PATH).toBe(".cursor/1. project/testing.md");
    expect(SETUP_TESTING_PROMPT_PATH).toBe(".cursor/1. project/testing.prompt.md");
    expect(SETUP_TESTING_PROMPTS_DIR).toBe(".cursor/1. project/testing");
  });
  ...
  it("setup documentation paths are under 1. project", () => {
    ...
    expect(SETUP_DOCUMENTATION_PROMPT_PATH).toBe(".cursor/1. project/documentation.prompt.md");
  });
  ...
  it("setup frontend and backend paths are under 1. project", () => {
    ...
    expect(SETUP_FRONTEND_PROMPT_PATH).toBe(".cursor/1. project/frontend.prompt.md");
    ...
    expect(SETUP_BACKEND_PROMPT_PATH).toBe(".cursor/1. project/backend.prompt.md");
    expect(SETUP_BACKEND_ANALYSIS_PATH).toBe(".cursor/1. project/backend-analysis.md");
  });

  it("all SETUP_* paths start with PROJECT_ROOT", () => {
    const setupPaths = [
      SETUP_DESIGN_DOC_PATH,
      SETUP_DESIGN_PROMPT_PATH,
      SETUP_ARCHITECTURE_DOC_PATH,
...
describe("getPromptPath", () => {
  it("returns ideas prompt path for ideas", () => {
    expect(getPromptPath("ideas")).toBe(".cursor/0. ideas/ideas.prompt.md");
  });

  it("returns project prompt path for project", () => {
    expect(getPromptPath("project")).toBe(".cursor/1. project/project.prompt.md");
  });

  it("returns path under 1. project for design, architecture, testing, documentation", () => {
    expect(getPromptPath("design")).toBe(".cursor/1. project/design.prompt.md");
    expect(getPromptPath("architecture")).toBe(".cursor/1. project/architecture.prompt.md");
    expect(getPromptPath("testing")).toBe(".cursor/1. project/testing.prompt.md");
    expect(getPromptPath("documentation")).toBe(".cursor/1. project/documentation.prompt.md");
  });

  it("returns frontend and backend prompt paths", () => {
    expect(getPromptPath("frontend")).toBe(".cursor/1. project/frontend.prompt.md");
    expect(getPromptPath("backend")).toBe(".cursor/1. project/backend.prompt.md");
  });

  it("returns empty string for unknown id (type-unsafe default)", () => {
    expect(getPromptPath("" as AnalyzeJobId)).toBe("");
  });
});
...
describe("getSetupPromptPath", () => {
  ...
  it("returns .prompt.md under 1. project for other keys", () => {
    expect(getSetupPromptPath("design")).toBe(".cursor/1. project/design.prompt.md");
    expect(getSetupPromptPath("architecture")).toBe(".cursor/1. project/architecture.prompt.md");
    expect(getSetupPromptPath("testing")).toBe(".cursor/1. project/testing.prompt.md");
    expect(getSetupPromptPath("documentation")).toBe(".cursor/1. project/documentation.prompt.md");
  });
});
