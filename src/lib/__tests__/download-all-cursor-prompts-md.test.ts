...
function file(
  ...
  return {
    relativePath: "8. worker/night-shift.prompt.md",
    path: ".cursor/8. worker/night-shift.prompt.md",
...
describe("buildCursorPromptsMarkdown", () => {
  ...
  it("includes one file as ## heading, Updated, and content", () => {
    const f = file({ path: ".cursor/8. worker/implement.prompt.md", content: "Implement phase." });
    const md = buildCursorPromptsMarkdown([f], exportedAt);
    expect(md).toContain("## .cursor/8. worker/implement.prompt.md");
